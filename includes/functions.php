...
function horarios(){
    include("db.php");
//valida que las horas disponibles segun la fecha seleccionada
    $fecha = $_POST['date'];
    $hora = "SELECT `hora` FROM citas WHERE `fecha` = '$fecha';";
    $resultado = mysqli_query($conexion, $hora);
...
function getNotifications() {
    ...
    $resultado = mysqli_query($conexion, "SELECT ct.id, ct.fecha, ct.hora, ct.estado, ct.fecha_registro, p.nombre, d.name, est.estado FROM citas ct INNER JOIN pacientes p ON ct.id_paciente = p.id INNER JOIN doctor d ON ct.id_doctor = d.id LEFT JOIN estado est ON ct.estado = est.id ORDER BY ct.fecha_registro DESC LIMIT 5;");
    if (mysqli_num_rows($resultado)>0)
    {    
        while ($dato = mysqli_fetch_assoc($resultado) ){
         $datos[] = $dato;
